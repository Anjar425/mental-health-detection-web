        ...
        <Card className="border-2 border-accent/20 shadow-lg">
            <CardHeader className="pb-4 p-4 sm:p-6">
                <div className="flex items-center gap-2 sm:gap-3 mb-2">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-xs sm:text-sm font-semibold">
                        {question.id}
                    </div>
                ...
                <div className="space-y-4 sm:space-y-6">
                        ...
                        <div className="p-3 sm:p-4 bg-accent/10 rounded-lg border border-accent/20">
                            <div className="flex items-start gap-2 sm:gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="font-semibold text-foreground mb-1 text-sm sm:text-base">{currentLabel.label}</div>
                    ...
                    <div className="grid grid-cols-2 gap-1.5 sm:gap-2 text-[10px] sm:text-xs">
                        {scaleLabels.map((scale) => (
                            <div onClick={() => onResponse(question.id, Number(scale.value))} key={scale.value} className="cursor-pointer flex items-center gap-1.5 sm:gap-2 p-1.5 sm:p-2 bg-muted/30 rounded">
                                ...
                                <div className="font-medium truncate">{scale.label}</div>
                            </div>
                        ))}
                    </div>
                    </div>
